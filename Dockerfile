...
RUN echo "build /etc/apt/sources.list.d/deb-src.list, and apt-get install" \
    ...
    && apt-get update && apt-get install -qqy --no-install-recommends \
           ...
           wget ccache distcc busybox apt-file net-tools traceroute iproute2 "bind9utils|bind9-dnsutils" debian-keyring redis-tools sbuild pbuilder \
...
RUN echo '# export CCACHE_DIR="$HOME/.ccache"' >> /home/builder/.bashrc \
     ...
     && echo '# export CCACHE_REMOTE_STORAGE="redis://172.17.0.1"' >> /home/builder/.bashrc \
     && echo 'export CCACHE_REMOTE_STORAGE="redis://dumm-pw-eechoocath7au5Zoen3oav3A@host.docker.internal"' >> /home/builder/.bashrc \
